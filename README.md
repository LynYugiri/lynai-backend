...
设备身份、canonical binary encoding、enrollment，以及 `/sync/changes`、blob 上传、purge 和 operation ACK 的 Ed25519 签名与幂等语义见 [`../lynai/doc/protocol-v1.md`](../lynai/doc/protocol-v1.md)，完整同步示例镜像在 [`internal/sync/testdata/sync-v1.json`](internal/sync/testdata/sync-v1.json)。所有同步写接口都强制要求当前登录 session 下已登记设备的有效签名，不提供 unsigned fallback。`GET /sync/status` 与 `GET /sync/index/status` 返回 `generation`、`indexRevision`、`minAvailableSeq`、基础 usage、limits 和 additive `capabilities`（index、selective/full purge、operation ACK）；旧客户端可忽略该字段。changes/blob 请求体超限返回 HTTP 413；changes JSON 必须只有一个顶层值、拒绝未知字段和尾随 JSON，strict delete 必须省略 `data`。为旧客户端兼容仍接受并忽略每条 change 上已废弃的 `deviceId`，服务端持久化和管理操作的审计设备始终只来自已验证签名。`GET /sync/changes` 的兼容字段 `latestSeq` 是本页安全游标（等于 `nextSince`），全局高水位另见 `globalLatestSeq`，每页还返回同一 repeatable-read、read-only 快照的 `indexRevision` 和 `minAvailableSeq`；旧客户端即使忽略 `hasMore` 也不会跳过 capped page。`since < minAvailableSeq` 返回 HTTP 409 `stale_cursor`，要求客户端从 index/current projection reseed。`GET /sync/blobs` 始终是有界列表；客户端必须在 `hasMore`/`truncated` 为 true 时使用 `nextAfter` 继续请求，`returnedCount` 和 `pageSize` 可用于检测旧版单页截断。blob 文件读取额外按服务端 blob 上限约束，发布时在支持目录 fsync 的文件系统上同步父目录。

云端索引接口为 `GET /sync/index/objects?category=...&expectedIndexRevision=...`（canonical、有效 UTF-8、最多 256 bytes 的 `after` keyset cursor，最大 500 条）和 `GET /sync/index/objects/:category/:objectId`；列表、详情和 purge preview 都使用非锁定的 repeatable-read、read-only 单一数据库快照，列表页和详情返回该快照的 `indexRevision`。管理接口为 `POST /sync/manage/purge/preview`、签名 `POST /sync/manage/purge`、`GET /sync/manage/operations` 和签名 `POST /sync/manage/operations/:id/ack`。现代 ACK body 携带 `operationId` 并必须与路径 ID 相等；仅为已发布旧客户端保留省略该字段的兼容。ACK 的 signed/body expected generation 必须同时等于当前 generation 和目标 operation 的 generation。selector 支持 `object`、`category`、`all`；每次 selective 或 full purge 都递增 generation，operation 记录递增后的 generation，并删除所有更早的 pending management operation，使 pending feed 只保留最新有效操作；被 supersede 的 purge request replay 仍保留并可按原请求精确重放。selective purge 保留未删除 change 的原 sequence 和用户 `lastSeq`，删除匹配 projection、change 和 blob refs，并把 `minAvailableSeq` 推进到当前 `lastSeq`；object purge 按每条历史 upsert 的 payload 重建当时的 object membership，delete 继承同记录最近一次 upsert 的 membership，因此 projection 已删除或记录后来迁移到其他 object 时仍能完整删除目标历史；category purge 按稳定 table/category 映射删除全部历史。full purge 清空用户 projection/change/blob refs并重置 `lastSeq/minAvailableSeq`。携带旧 generation 的客户端统一收到 HTTP 409 `generation_mismatch`，不会先按旧 cursor、index revision 返回其他冲突；切换到新 generation 后，低于 `minAvailableSeq` 的 selective-purge cursor 才返回 `stale_cursor`。两者都不生成同步 tombstone，不物理删除 blob 或 blob metadata，也不删除尚未过期的 request replay；使用 purge 原始 `requestId`、body 和原 expected generation 重试时仍精确重放最初响应。所有索引分页和 purge 必须携带当前 `expectedIndexRevision`；当前 generation 下 revision 不匹配返回 HTTP 409。request ID 内容冲突返回 HTTP 409 `replay_conflict`，不再误报为 index revision conflict。
...
同步变更 allowlist 包含版本化的 `shared_settings`、`synced_model_configs`、`plugin_files`、`plugin_settings`、`plugin_config`，以及 `knowledge_bases`、`knowledge_categories`、`knowledge_entries`、`knowledge_sources`、`knowledge_explanations` 逻辑域。知识 payload 必须提供客户端落库所需的最小字段类型：必需文本、bool、signed 64-bit int 和带时区的 RFC3339 时间不得缺失或错型，可空文本必须为 string/null；所有 ID 最多 256 bytes，category alias 必须匹配 `^[a-z][a-z0-9_-]{0,31}$`。知识记录在管理索引中统一归入 `knowledge` category：`knowledge_bases` 以自身 `recordId` 为 object ID，但 `global` 是 legacy settings 保留 ID，不得用作知识库 ID；知识子记录必须携带非空 `data.knowledgeBaseId` 并以其作为 object ID，source/explanation 还必须携带非空 `entryId`。服务端在同一 upload 应用完全部 change 后验证最终 projection 的 base/category/entry/source/explanation 父子归属和全局 alias 唯一性，因此同批次父子 upsert 不依赖上传顺序。`knowledge_categories.isDefault` 和 `knowledge_settings` 已不属于现行知识库领域语义：category payload 不再要求 `isDefault`，但滚动升级期间仍接受并忽略旧客户端携带的 bool 值；服务端暂时继续接受、存储和同步 record/object ID 固定为 `global` 的 legacy `knowledge_settings`，并保留其旧 payload 类型校验以避免旧客户端 batch poisoning，但不再按其中的默认 ID 校验最终 knowledge projection。为兼容仍会读取该 legacy settings：purge `knowledge/<baseId>` 时，若当前 settings 引用该 base，则 preview 和实际 purge 一并计入并删除 settings projection、全部历史和 blob 引用，避免旧客户端 reseed 得到悬空默认值；settings 引用其他 base 或未设置默认值时保持不变。运行时 `plugin_storage` 有意保持设备本地，不进入云同步。客户端只上传共享设置投影和用户明确启用的非托管 Provider 非秘密配置；完整 `app_settings`、API key、secret 引用和后端托管 Relay 配置不属于云同步 payload。
...
Canonical generation 参数使用 camelCase：`maxTokens`、`temperature`、`topP`、`presencePenalty`、`frequencyPenalty`、`seed`、`stop` 和 `user`。请求未提供时使用管理员保存的模型默认值；OpenAI 接收全部字段，Anthropic 将 `stop` 转为 `stop_sequences`，Ollama 将 `stop` 放入 `options.stop`。`reasoning.budgetTokens` 只支持 Anthropic；OpenAI 和 Ollama 请求携带该字段会返回明确的参数错误，避免静默忽略预算。OpenAI 请求还可通过 `extraParams` 传递供应商扩展字段，但不能覆盖 relay 生成的核心字段；流式请求总会把 `stream_options.include_usage` 设为 `true`，并保留同一 object 中的其他扩展字段。

非流式响应固定为 `{"message":{"role":"assistant","content":"...","reasoning":"...","toolCalls":[]},"finishReason":"stop"}`。流式响应继续使用 SSE `event: chunk`，`data` 保留 canonical delta 的 `content`、`reasoning`、`toolCalls`、`finishReason`、`done` 和 `error` 字段，并增量提供 `sequence`（每条流从 1 单调递增）、稳定的 `responseId` 及 `type`（`content`、`reasoning`、`delta`、`tool_calls`、`completed` 或 `error`）。`type` 的优先级是 `error`、`tool_calls`、`completed`、内容类型；因此含工具调用的终止 delta 使用 `type: "tool_calls"`，同时仍以 `done: true` 表示流已结束。上游提供 token 统计时，终止事件还包含 camelCase `usage.inputTokens`、`usage.outputTokens` 和 `usage.totalTokens`；Anthropic 的 input/total 会计入 cache creation 和 cache read input tokens。错误终止事件在保留字符串 `error` 的同时包含 `errorInfo.type` 和 `errorInfo.message`。现有只读取旧字段或只监听 `chunk` 的客户端保持兼容。OpenAI tools 完整支持请求、历史 tool call/result、非流式和流式返回；Anthropic 与 Ollama tools 在其原生协议可表达的范围内转换，不能表达的 `toolChoice` 或畸形工具参数会明确报错。
